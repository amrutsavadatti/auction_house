  ...
  let sort = (sorter) => {
    return new Promise((resolve, reject) => {
      if(sorter == "finalSalePrice"){
        pool.query("SELECT * FROM AuctionHouse.Items WHERE boughtByBuyer IS NOT NULL ORDER BY finalSalePrice DESC", [], (error, rows) => {
          if (error) { 
            return reject(error) 
          }
          if ((rows)) {
      ...
      if(sorter == "publishDate"){
        pool.query("SELECT * FROM AuctionHouse.Items WHERE boughtByBuyer IS NOT NULL ORDER BY publishDate DESC", [], (error, rows) => {
          if (error) { 
            return reject(error) 
          }
          if ((rows)) {
      ...
      if(sorter == "expirationDate"){
        pool.query("SELECT * FROM AuctionHouse.Items WHERE boughtByBuyer IS NOT NULL ORDER BY expirationDate DESC", [], (error, rows) => {
          if (error) { 
            return reject(error) 
          }
          if ((rows)) {
