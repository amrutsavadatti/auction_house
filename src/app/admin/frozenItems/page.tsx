...
export default function Home() {
    ...
    const getUnfreezeRequests = async () => {
      try {
        const response = await fetch(' https://zseolpzln7.execute-api.us-east-2.amazonaws.com/Initial/getUnfreezeRequests', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (!response.ok) {
          throw new Error('Failed to get unfreeze requested items');
        }

        const data = await response.json();
        setUnfreezeRequests(data.items);
        console.log("Number of items:", items.length);
        console.log("First item:", items[0]);  // Log only the first item to inspect its structure
    ...
    return (
        <div className='flex flex-col justify-center '>
            ...
            <h1 style={{display: 'flex', justifyContent: 'center', fontWeight: 'bold', fontSize: '40px'}}>List of Unfreeze Requests</h1>
            ...
            <div className="m-4 bg-accent-content p-4 rounded-xl">
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                      {unfreezeRequests.map((item, index) => (
                      <tr key={index} className="border-b border-gray-700">
                          <th>
                          <label>
                              <h2>{index + 1}</h2>
                          ...
                          <th>
                              <button className="btn btn-outline btn-info btn-xs " onClick = {() => handleUnfreezeItem(item.seller, item.itemName)} >unfreeze item</button>
                          </th>
                      </tr>
                      ))}
                  </tbody>
